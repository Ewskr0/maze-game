...
void player::move(point2d new_pos) { pos = new_pos; }

void player::displayHP() { std::cout << "HP: " << hp << std::endl; }

bool player::isAlive() { return hp > 0; }

void player::applyFieldEffect(field_effect f) {
  switch (f.state) {
...
void player::observe(maze m_maze) {
...
field_ptr player::move(maze m_maze) {
